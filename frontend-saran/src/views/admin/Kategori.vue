            ...
            <div class="container">
             <div class="max-w-lg mx-auto shadow-md hover:shadow-lg rounded-lg overflow-hidden">
                 <div class="py-8 px-6 md:px-10 lg:px-12">
                     <h2 class="text-2xl lg:text-3xl font-semibold mb-4 text-center">Tambah Kategori</h2>
                     <form @submit.prevent="saveKategori" class="space-y-4" method="post">
                         <div>
                             <input type="text" v-model="model.kategori.nama_kategori" id="kategori" placeholder="Masukkan Kategori" class="w-full bg-white rounded-lg border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-2 px-4 duration-200 ease-in-out">
                         </div>
                         <div>
                ...
                <table class="w-full table-auto">
                    ...
                    <tbody>
                        <tr v-for="(data, index) in this.data" :key="index" >
                            ...
                            <td class="p-5 w-72 text-center">{{ data.nama_kategori}}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    export default {
        ...
        methods: {
            saveKategori() {
                ...
                if (!this.model.kategori.nama_kategori) {
                    Swal.fire({
                    icon: "error",
                    title: "Gagal menambah data",
                    text: "Harap isi field form kategori"
                    });
                    return
                }
                ...
                var mythis = this;
                axios.post(`${urlapi}/tambah-data/kategori`, this.model.kategori, config).then(res => {
            ...
            getKategori() {
                ...
                axios.get(`${urlapi}/kategoriall`, config)
                .then(res => {
